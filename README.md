...
XCTU の代替として、XBee 2 台の最低限のペアリング設定だけを行う静的 Web アプリです。Chrome / Edge の Web Serial API を使い、`ID`、`CE`、`DL` のみを書き込みます。

## Quick start

公開ページは `docs/index.html` を想定しています。GitHub Pages の URL は通常次の形式です。

`https://<GitHubユーザー名>.github.io/XbeePairingGUI/`

公開時は実際の GitHub Pages URL に合わせて、この README 内の `<GitHubユーザー名>` を置き換えてください。

1. Chrome または Edge で GitHub Pages URL を開く
2. XBee を 2 台 USB シリアルとして接続する
